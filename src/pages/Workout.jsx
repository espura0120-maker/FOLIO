...
export default function Workout() {
  ...
  function handleLog(e) {
    ...
    add({
      session: { ...session, duration_mins: +session.duration_mins || 0, total_volume: volume },
      exercises: validEx.map(ex => ({ name: ex.name, sets: +ex.sets||0, reps: +ex.reps||0, weight: +ex.weight||0 }))
    })
    ...
    <div className="fade-up">
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <form onSubmit={handleLog} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <select value={session.type} onChange={setS('type')} style={{ flex: 1 }}>
          ...
          <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '12px 14px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 65px 65px 100px 28px', gap: 6, marginBottom: 8 }}>
              {['Exercise','Sets','Reps',`Weight (${weightUnit})`, ''].map((h, i) => (
                <div key={i} style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</div>
              ))}
            </div>
            {exercises.map((ex, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 65px 65px 100px 28px', gap: 6, marginBottom: 6 }}>
                ...
                <input value={ex.weight} onChange={setE(i,'weight')} type="number" min={0} step="0.5" placeholder="0" style={{ fontSize: 13, padding: '7px 9px' }} />
      ...
      <Card>
        ...
        {sessions.length === 0
          ...
          : sessions.map(s => {
            const exStr = s.exercises?.map(e => `${e.name} ${e.sets}×${e.reps}${e.weight ? ` @ ${e.weight}${weightUnit}` : ''}`).join(' · ') || ''
            return (
              <ListItem key={s.id} icon={s.type[0]} iconBg="rgba(138,110,216,0.15)"
                ...
                right={<span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 13, color: 'var(--purple2)' }}>
                  {s.total_volume > 0 ? `${(+s.total_volume).toLocaleString()} ${weightUnit}` : s.type}
                </span>}
