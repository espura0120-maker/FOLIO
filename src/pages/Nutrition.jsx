...
export default function Nutrition() {
  ...
  const { logs, add, remove, totalCalories, totalProtein, calGoal } = useFoodLogs()
  ...
  const dateLogs = logs.filter(f => f.date === dateKey)
  const dateCalories = dateLogs.reduce((s, f) => s + (+f.calories || 0), 0)
  const dateProtein  = dateLogs.reduce((s, f) => s + (+f.protein_g || 0), 0)
  ...
  async function analyzeImage(e) {
    ...
    reader.onload = async ev => {
      ...
      if (!apiKey) {
        setAi({ loading: false, result: null, error: 'Add VITE_ANTHROPIC_API_KEY to use AI analysis.' })
        return
      }
      try {
        const resp = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: 'user', content: [
              ...
              { type: 'text', text: 'Analyze this food. Respond ONLY in JSON: {"food":"name","calories":number,"protein":number,"carbs":number,"fat":number,"notes":"one sentence"}. No markdown.' }
            ]}]
          })
        })
        ...
        setAi({ loading: false, result: null, error: 'Could not analyze image. Try again.' })
      }
    }
  ...
  function logAiResult() {
    add({
      name: ai.result.food,
      calories: ai.result.calories,
      protein_g: ai.result.protein,
      carbs_g: ai.result.carbs || 0,
      fat_g: ai.result.fat || 0,
      meal: form.meal,
      ai_analyzed: true,
      date: dateKey,
    })
        ...
        <div style={{ background: 'var(--bg3)', borderRadius: 'var(--radius)', padding: '14px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Button size="sm" onClick={() => {
              ...
              if (v > 0) updateProfile({ cal_goal: v })
            }}>Set</Button>
          </div>
        </div>
      </Grid>
        ...
        <Card>
          ...
          {ai.result && (
            <div style={{ background: 'rgba(61,184,138,0.08)', border: '1px solid rgba(61,184,138,0.2)', borderRadius: 'var(--radius)', padding: '14px 16px' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <select value={form.meal} onChange={set('meal')} style={{ fontSize: 12, padding: '6px 8px' }}>
                  {MEALS.map(m => <option key={m} value={m}>{m.charAt(0).toUpperCase()+m.slice(1)}</option>)}
                </select>
                <Button size="sm" variant="teal" onClick={logAiResult} style={{ flex: 1 }}>+ Add to Log</Button>
                <Button size="sm" onClick={() => setAi({ loading: false, result: null, error: null })}>x</Button>
              </div>
            </div>
          )}
        </Card>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--coral2)', marginTop: 2 }}>
              {dateCalories} kcal · {dateProtein.toFixed(1)}g protein
            </div>
          </div>
        ...
        <CardTitle>
          {isSelectedToday ? "Today's Log" : `Log for ${format(selectedDate, 'MMM d')}`}
        </CardTitle>

        {dateLogs.length === 0
          ? <EmptyState icon="🍽" message={isFuture ? "Can't log future dates" : "Nothing logged for this day."} />
          : MEALS.map(meal => {
            const mFoods = dateLogs.filter(f => f.meal === meal)
            if (!mFoods.length) return null
            return (
              <div key={meal} style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--text3)', margin: '10px 0 6px' }}>
                  {MEAL_ICON[meal]} {meal}
                </div>
                {mFoods.map(f => (
                  <ListItem key={f.id}
                    icon={f.ai_analyzed ? '🤖' : '🍽'}
                    iconBg="rgba(217,100,74,0.12)"
                    name={f.name}
                    sub={`P: ${f.protein_g||0}g · C: ${f.carbs_g||0}g · F: ${f.fat_g||0}g${f.ai_analyzed ? ' · AI' : ''}`}
