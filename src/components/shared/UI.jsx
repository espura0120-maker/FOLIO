...
export function Card({ children, style, onClick }) {
  return (
    <div onClick={onClick} style={{
      background:'#1c1e2b', border:'1px solid rgba(255,255,255,0.07)',
      borderRadius:18, padding:'16px 18px',
      cursor:onClick?'pointer':undefined, transition:'border-color 0.18s',
      fontFamily:F, ...style,
    }}
...
export function StatCard({ label, value, sub, color='#f5c842', accent }) {
  return (
    <div style={{ background:'#1c1e2b', border:'1px solid rgba(255,255,255,0.07)', borderRadius:14, padding:'14px 16px', fontFamily:F }}>
...
export function Button({ children, variant='default', size='md', fullWidth, loading, disabled, onClick, type='button', style }) {
  ...
  const vars = {
    default: { background:'#262838', border:'1px solid rgba(255,255,255,0.10)', color:'rgba(255,255,255,0.70)' },
  ...
  return (
    <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:20 }}>
      <div>
        ...
        {sub && <p style={{ fontSize:13, color:'rgba(255,255,255,0.38)', fontFamily:F }}>{sub}</p>}
      </div>
      {action}
    </div>
  )
...
export function CardTitle({ children }) {
  return (
    <div style={{ fontFamily:F, fontSize:11, fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:12 }}>
      {children}
    </div>
  )
}
...
export function ListItem({ icon, iconBg, name, sub, right, onDelete }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', background:'#222435', borderRadius:10, marginBottom:6, border:'1px solid rgba(255,255,255,0.06)', fontFamily:F }}>
      {icon && (
        <div style={{ width:34, height:34, borderRadius:8, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background:iconBg||'#2a2d40', fontSize:16 }}>{icon}</div>
      )}
